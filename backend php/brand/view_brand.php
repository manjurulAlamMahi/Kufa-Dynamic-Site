                        ...
                        <div class="card-body">
                            <table class="table table-bordered">
                                <tr>
                                    ...
                                    <td class="user_header_text">brand</td>
                                    <td class="user_header_text">Status</td>
                                    <td class="user_header_text">Action</td>
                                </tr>
                                <?php foreach($select_rslt as $key => $brand){ ?>
                                <tr>
                                    <td class="vv">#<?= $key+1 ?></td>
                                    <td class="vv"><img width="100px" src="/L/assets/photos/index/brand/<?= $brand['brand'] ?>" alt="<?= $brand['brand'] ?>"></td>
                                    <td class="vv">
                                        <a href="act_dct_brand.php?id=<?=$brand["id"]?>"
                                        class="btn btn-<?=($brand['status'] == 1? 'success':'secondary') ?> my_btn">
                                        <?=($brand['status'] == 1? 'Active':'Deactive') ?>
                                    </td>
                                    <td class="vv">
                                        <button type="button" name="delete_brand.php?id=<?=$brand["id"]?>"
