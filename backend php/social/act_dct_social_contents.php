...
$select = "SELECT * FROM social_content WHERE id=$id";
$select_rslt = mysqli_query($db_connect,$select);
$after_assoc = mysqli_fetch_assoc($select_rslt);

// COUNT ACTIVE STATUS 
$count = "SELECT COUNT(*) as total FROM social_content WHERE status=1";
...
if($after_assoc['status'] == 1){
    ...
    if($after_assoc_count['total'] == 1){
        $_SESSION['limit'] = 'Minimum One Icon Must Active!';
        header('location:view_social_contents.php');
    }
    else{
        $update_content_act = "UPDATE social_content SET status=0 WHERE id=$id";
    ...
    if($after_assoc_count['total'] == 4){
        $_SESSION['limit'] = 'Maximum Icon Activated!';
        header('location:view_social_contents.php');
    }
    else{
        $update_content_act = "UPDATE social_content SET status=1 WHERE id=$id";
        $update_content_act_rslt = mysqli_query($db_connect,$update_content_act);
