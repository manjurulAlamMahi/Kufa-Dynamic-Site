...
<?php 
    ...
    $select = "SELECT * FROM social_content";
                        ...
                        <div class="card-body">
                            <table class="table table-bordered">
                                <tr>
                                    ...
                                    <td class="user_header_text">Link</td>
                                    <td class="user_header_text">Status</td>
                                    <td class="user_header_text">Action</td>
                                </tr>
                                <?php foreach($select_rslt as $key => $social_contents){ ?>
                                <tr>
                                    <td class="vv">#<?= $key+1 ?></td>
                                    <td class="vv"><i style="color:#333; font-size:22px;" class="fa <?= $social_contents['icon']; ?>"></i></td>
                                    <td class="vv"><a href="<?= $social_contents['link']; ?>"><?= substr($social_contents['link'],0,20) ?>.....</a></td>
                                    <td class="vv">
                                        <a href="act_dct_social_contents.php?id=<?=$social_contents["id"]?>"
                                        class="btn btn-<?=($social_contents['status'] == 1? 'success':'secondary') ?> my_btn">
                                        <?=($social_contents['status'] == 1? 'Active':'Deactive') ?>
                                    </td>
                                    <td class="vv">
                                        <button type="button" name="delete_social_contents.php?id=<?=$social_contents["id"]?>"
