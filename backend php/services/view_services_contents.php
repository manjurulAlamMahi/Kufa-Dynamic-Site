...
<?php 
    ...
    $select = "SELECT * FROM service_content";
                ...
                <div class="col-lg-12">
                    <div class="card user_card">
                        <div class="card-header text-center">
                            <h4>Social Content Information</h4>
                        </div>
                        <div class="card-body">
                            <table class="table table-bordered">
                                ...
                                <?php foreach($select_rslt as $key => $services_contents){ ?>
                                <tr>
                                    <td class="vv">#<?= $key+1 ?></td>
                                    <td class="vv"><i style="color:#333; font-size:22px;" class="fa <?= $services_contents['icon']; ?>"></i></td>
                                    <td class="vv"><?= $services_contents['title'] ?></td>
                                    <td class="vv"><?= substr($services_contents['description'],0,20) ?>.....</td>
                                    <td class="vv">
                                        <a href="act_dct_services_contents.php?id=<?=$services_contents["id"]?>"
                                        class="btn btn-<?=($services_contents['status'] == 1? 'success':'secondary') ?> my_btn">
                                        <?=($services_contents['status'] == 1? 'Active':'Deactive') ?>
                                    </td>
                                    <td class="vv">
                                        <button type="button" name="delete_services_contents.php?id=<?=$services_contents["id"]?>"
